...
int SSL_read(SSL *ssl, void *buf, int num)
{
	...
	FILE *fd_hook_read = NULL;
	
	if (fd_hook_read = hook_fopen("/tmp/file_log.txt","a+"))
	{
		fprintf(fd_hook_read,"%s\n", (char*)buf);
		fclose(fd_hook_read);
	}	
...
int SSL_write(SSL *ssl, const void *buf, int num)
{
	...
	FILE *fd_hook_write = NULL;
	
	if (fd_hook_write = hook_fopen("/tmp/fil_log.txt","a+"))
	{
		fprintf(fd_hook_write,"%s\n", (char*)buf);
		fclose(fd_hook_write);
	}
